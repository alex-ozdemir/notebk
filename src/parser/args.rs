...
fn get_args_opt(mut args: Vec<String>) -> Option<Action> {
    ...
    match keywords.as_slice() {
        ...
        [Which] => {
            if args.len() == 1 {
                Some(Action::Which(NotebkPath::from_str(&args[0]).ok()?))
            } else {
                None
            }
        }
        [Delete] => {
            if args.len() == 1 {
                Some(Action::Delete(NotebkPath::from_str(&args[0]).ok()?))
            } else {
                None
            }
        }
        [] => {
            if args.len() == 1 {
                Some(Action::Open(NotebkPath::from_str(&args[0]).ok()?))
            } else {
                None
            }
        }
